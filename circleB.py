...
def drawing_pic(state, party, votes):
    ...
    x_start = (votes_drawn % 10) * 30 - 150  # Adjust the spacing and starting position as needed
    y_start = 200 - (votes_drawn // 10) * 30

    # Draw the votes for the state
    for i in range(votes):
        x = x_start + (i % 10) * 30
        y = y_start - (i // 10) * 30  # Corrected y-coordinate calculation
        draw_vote(party, x, y)
        votes_drawn += 1
        votes_remaining -= 1
